...
export function createCityRoadsideProp(index) {
  ...
  if (kind === 0) {
    ...
    const barrier = new THREE.Mesh(
      new THREE.BoxGeometry(2.8, 1.25, 0.9),
      concrete,
    );
    const strip = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.12, 0.09), glow);
    strip.position.set(0, 0.05, 0.47);
    const boltMat = new THREE.MeshStandardMaterial({ color: "#888", metalness: 0.85, roughness: 0.2 });
    ...
    const topChamfer = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.08, 0.6), new THREE.MeshStandardMaterial({ color: "#555", roughness: 0.9 }));
    ...
    const hydrantBody = new THREE.Mesh(
      new THREE.CylinderGeometry(0.28, 0.32, 1.0, 12),
      rust,
    );
    ...
    const coneBody = new THREE.Mesh(
      new THREE.CylinderGeometry(0.05, 0.32, 0.9, 12),
      glow,
    );
