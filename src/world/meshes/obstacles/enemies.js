...
export function createRaider(models, tireMat) {
  ...
  const vRustMat = new THREE.MeshStandardMaterial({
    color: bodyColor,
    roughness: 0.85,
    metalness: 0.5,
  });
  const vDarkMetal = new THREE.MeshStandardMaterial({
    color: armorColor,
    roughness: 0.7,
    metalness: 0.7,
  });
  ...
  const vChromeMat = new THREE.MeshStandardMaterial({
    color: "#aaa",
    metalness: 0.95,
    roughness: 0.15,
  });
  ...
  if (models["raider"]) {
    ...
    const gunMuzzle = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.14, 0.25, 8), vChromeMat);
    ...
    for (let sp = 0; sp < spikeCount; sp++) {
      const spikeGeo = new THREE.ConeGeometry(
        0.06 * vScale,
        (variant === 1 ? 0.55 : 0.4) * vScale,
        5,
      );
      const spike = new THREE.Mesh(spikeGeo, vDarkMetal);
      spike.position.set(
        (-spikeCount * 0.22 + sp * 0.44) * vScale,
        (0.7 + (sp % 2) * 0.1) * vScale,
        2.15 * vScale,
      );
    ...
    if (variant === 0) {
      const skullBase = new THREE.Mesh(
        new THREE.SphereGeometry(0.28 * vScale, 8, 6),
        new THREE.MeshStandardMaterial({ color: "#e8d5c0", roughness: 0.7 }),
      );
      skullBase.position.set(0, 1.1 * vScale, 1.7 * vScale);
      skullBase.scale.set(1, 0.8, 0.9);
      const skullJaw = new THREE.Mesh(
        new THREE.BoxGeometry(0.28 * vScale, 0.14, 0.18),
        new THREE.MeshStandardMaterial({ color: "#d4c0a8", roughness: 0.8 }),
      );
      skullJaw.position.set(0, 0.92 * vScale, 1.78 * vScale);
      const skullEyeL = new THREE.Mesh(
        new THREE.SphereGeometry(0.08 * vScale, 6, 5),
        new THREE.MeshStandardMaterial({
          color: "#ff0000",
          emissive: "#ff4400",
      ...
      for (const side of [-1, 1]) {
        const blade = new THREE.Mesh(
          new THREE.BoxGeometry(0.45 * vScale, 0.03, 0.55),
          new THREE.MeshStandardMaterial({ color: "#ccc", metalness: 0.9, roughness: 0.2 }),
        );
    ...
    for (const ex of [-0.85 * vScale, 0.85 * vScale]) {
      const pipeOuter = new THREE.Mesh(
        new THREE.CylinderGeometry(0.12 * vScale, 0.16 * vScale, 0.3 * vScale, 10),
        vChromeMat,
      );
      pipeOuter.position.set(ex, 0.35 * vScale, -1.8 * vScale);
      pipeOuter.rotation.x = Math.PI / 2;
      const pipeInner = new THREE.Mesh(
        new THREE.CylinderGeometry(0.06 * vScale, 0.1 * vScale, 0.8 * vScale, 8),
        vDarkMetal,
      );
...
export function createTower(models, rustMat, darkMetal) {
  ...
  if (models["tower"]) {
    ...
    const beaconMesh = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: "#ff3300" }),
    );
    ...
    const midRing = new THREE.Mesh(new THREE.TorusGeometry(1.65, 0.15, 8, 16), darkMetal);
    midRing.position.set(0, 3.0, 0);
    midRing.rotation.x = Math.PI / 2;
    group.add(midRing);

    const collar = new THREE.Mesh(new THREE.CylinderGeometry(1.75, 1.5, 1.0, 10), darkMetal);
    ...
    for (let i = 0; i < 14; i++) {
      ...
      const r = 1.65;
      spike.position.set(Math.cos(ang) * r, 5.95 + Math.random() * 0.3, Math.sin(ang) * r);
    ...
    for (let sk = 0; sk < 5; sk++) {
      ...
      const skull = new THREE.Mesh(
        new THREE.SphereGeometry(0.22 + Math.random() * 0.06, 7, 5),
        skullMat,
      );
      skull.position.set(Math.cos(skullAng) * skullR, skullHeight, Math.sin(skullAng) * skullR);
    ...
    for (let dmg = 0; dmg < 6; dmg++) {
      const rebar = new THREE.Mesh(
        new THREE.BoxGeometry(0.08, 1.8 + Math.random() * 0.8, 0.08),
        darkMetal,
      );
      rebar.position.set(
        (Math.random() - 0.5) * 2.2,
...
export function createMutant(_models) {
  const group = new THREE.Group();
  const variant = Math.floor(Math.random() * 3);
  ...
  const belly = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      color: variant === 0 ? "#6a4a3a" : fleshColor,
      roughness: 0.85,
    }),
  );
  ...
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.7 * mScale, 8, 6), fleshMat);
  ...
  for (const ax of [-1, 1]) {
    const shoulder = new THREE.Mesh(new THREE.SphereGeometry(shoulderW * mScale, 7, 5), fleshMat);
  ...
  for (const ax of [-1, 1]) {
    ...
    if (variant === 1) {
      for (let b = 0; b < 2; b++) {
        const blade = new THREE.Mesh(
          new THREE.ConeGeometry(0.06 * mScale, 0.9 * mScale, 4),
          boneMat,
        );
  ...
  if (variant === 0) {
    for (let sp = 0; sp < 6; sp++) {
      const spine = new THREE.Mesh(
        new THREE.ConeGeometry(0.05 * mScale, 0.55 * mScale, 4),
        boneMat,
      );
      spine.position.set(
        (Math.random() - 0.5) * 0.4 * mScale,
        (1.4 + sp * 0.28) * mScale,
        -0.55 * mScale,
      );
