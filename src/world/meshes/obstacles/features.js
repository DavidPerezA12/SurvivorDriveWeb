...
export function createRamp(models) {
    ...
    const steelMat = new THREE.MeshStandardMaterial({ color: "#444", metalness: 0.7, roughness: 0.4 });
    ...
    const rampEdgeL = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.3, 5.5),
      steelMat,
    );
    ...
    const baseGirder1 = new THREE.Mesh(
      new THREE.BoxGeometry(0.12, 0.8, 1.6),
      steelMat,
    );
    ...
    for (const bx of [-1.4, 1.4]) {
      const strip = new THREE.Mesh(
        new THREE.BoxGeometry(0.25, 0.1, 2.0),
        hazardMat,
      );
    ...
    for (let s = 0; s < 6; s++) {
      const stripe = new THREE.Mesh(
        new THREE.BoxGeometry(0.15, 0.04, 0.5),
        hazardMat,
      );
