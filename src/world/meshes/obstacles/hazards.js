...
export function createOilSpill(_models) {
  ...
  for (let s = 0; s < 5; s++) {
    ...
    shimmer.position.set((Math.random() - 0.5) * 2.4, 0.035, (Math.random() - 0.5) * 3.4);
    group.add(shimmer);
  }
...
export function createMine(_models) {
  const group = new THREE.Group();
...
export function createPothole(_models) {
  const group = new THREE.Group();
