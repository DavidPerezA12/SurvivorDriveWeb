...
export function createBarrier(models) {
  ...
  if (models["barrier"]) {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(3.4, 1.3, 1.4), barrierMat);
    ...
    const strip = new THREE.Mesh(new THREE.BoxGeometry(3.3, 0.25, 0.1), stripMat);
    ...
    for (let s = 0; s < 6; s++) {
      ...
      stain.position.set(-1.5 + Math.random() * 3.0, 0.2 + Math.random() * 0.8, 0.72);
      group.add(stain);
    }
...
export function createMilitaryBarrier(_models) {
  const group = new THREE.Group();
  const milMat = new THREE.MeshStandardMaterial({
    color: "#3a4a2a",
    roughness: 0.85,
    metalness: 0.4,
  });
  ...
  for (let c = 0; c < 3; c++) {
    const camo = new THREE.Mesh(
      new THREE.BoxGeometry(0.6 + Math.random() * 0.9, 0.25, 0.04),
      camoMat,
    );
...
export function createHalfGate(_models) {
  const group = new THREE.Group();
...
export function createRock(_models) {
  const group = new THREE.Group();
  ...
  for (let p = 0; p < 3; p++) {
    ...
    pebble.position.set((Math.random() - 0.5) * 1.6, 0.04, (Math.random() - 0.5) * 1.6);
