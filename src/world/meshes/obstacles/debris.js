...
export function createWreck(models, tireMat) {
  ...
  if (models["wreck"]) {
    ...
    const glow = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: "#ff4400", transparent: true, opacity: 0.7 }),
    );
    emberLight.add(glow);
    ...
    const glow = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.12, 0.4), fireMat);
    ...
    for (let g = 0; g < 10; g++) {
      const shard = new THREE.Mesh(
        new THREE.BoxGeometry(0.08 + Math.random() * 0.2, 0.06 + Math.random() * 0.2, 0.02),
        glassMat,
      );
      shard.position.set(
        (Math.random() - 0.5) * 2.0,
    ...
    for (let d = 0; d < 6; d++) {
      const debris = new THREE.Mesh(
        ...
        d % 2 === 0
          ? debrisMat
          : new THREE.MeshStandardMaterial({ color: "#555", metalness: 0.6, roughness: 0.5 }),
      );
      debris.position.set(
        (Math.random() - 0.5) * 3.0,
        Math.random() * 0.4,
...
export function createDebris(_models) {
  const group = new THREE.Group();
  const debrisMats = [
...
export function createFallenSign(_models) {
  const group = new THREE.Group();
...
export function createScrap(models, darkMetal) {
  ...
  if (models["scrap"]) {
    ...
    const beam = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.8, 0.2), darkMetal);
    ...
    const plate = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.08, 0.6), plateMat);
